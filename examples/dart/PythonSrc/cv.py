...
class CrossValidate:
    ...
    def __init__(self, data_file, fold, debug_file = "", cv_file = ""):
...
def test(train_data_file, test_data_file):
    ...
    #clf = AdaBoostClassifier()
    #clf = GradientBoostingClassifier(max_depth=3) # Reasonbale precision for 0, recall/precision for 2
    #clf = BaggingClassifier(random_state=0, n_estimators=100) # Good one
    clf = BaggingClassifier(random_state=0, n_estimators=100)
    ...
    print("Confusion Matrix = ", confusion_matrix(all_test_problems.get_label_list(), predicted_labels))
